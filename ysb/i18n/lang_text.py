...
UI_KO_EN.update({
    ...
    '옵션, 단축키, 매크로, 글꼴 프리셋, 번역 프롬프트, 단어장 같은 작업환경 캐시를 클라우드에 백업합니다. 불러온 폰트 파일 자체는 용량과 라이선스 문제를 피하기 위해 백업하지 않습니다.': 'Back up work-environment cache such as options, shortcuts, macros, font presets, translation prompts, and glossary to the cloud. Imported font files themselves are not included to avoid size and license issues.',
    '옵션, 단축키, 매크로, 프리셋, 프롬프트, 단어장 같은 작업환경 캐시를 백업합니다. 불러온 폰트 파일 자체는 백업하지 않습니다. API 키는 체크박스로 별도 선택하며, 포함 시 업로드 전 암호화와 불러오기 시 복호화가 필수입니다.': 'Back up work-environment cache such as options, shortcuts, macros, presets, prompts, and glossary. Imported font files themselves are not included. API keys are selected separately with a checkbox, and if included, encryption before upload and decryption on restore are required.',
...
UI_KO_EN.update({
    ...
    '전체 페이지의 OCR 분석 범위 지정': 'Set OCR Analysis Region for All Pages',
    ...
    '빠른 OCR 설정 저장 완료': 'Quick OCR Settings Saved',
